...
esp_err_t tca9535_create(const tca9535_config_t *config, tca9535_handle_t *handle)
{
    ...
    // 复制I2C设备描述符
    memcpy(&dev->i2c_dev, &config->i2c_dev, sizeof(i2c_dev_t));

    // 创建I2C设备互斥锁
    esp_err_t ret = i2c_dev_create_mutex(&dev->i2c_dev);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "创建I2C设备互斥锁失败: %s", esp_err_to_name(ret));
        free(dev);
        return ret;
    }
    ...
    *handle = dev;
    ...
    ESP_LOGI(TAG, "TCA9535设备创建成功 (地址: 0x%02X, 端口: %d)", 
             dev->i2c_dev.addr, dev->i2c_dev.port);
...
esp_err_t tca9535_read_register(tca9535_handle_t handle, tca9535_reg_t reg, uint8_t *data)
{
    ...
    esp_err_t ret = i2c_dev_read_reg(&dev->i2c_dev, reg, data, 1);
    if (ret != ESP_OK) {
...
esp_err_t tca9535_write_register(tca9535_handle_t handle, tca9535_reg_t reg, uint8_t data)
{
    ...
    esp_err_t ret = i2c_dev_write_reg(&dev->i2c_dev, reg, &data, 1);
    if (ret != ESP_OK) {
...
esp_err_t tca9535_read_register_pair(tca9535_handle_t handle, tca9535_reg_t reg, tca9535_register_t *data)
{
    ...
    esp_err_t ret = i2c_dev_read_reg(&dev->i2c_dev, reg, read_data, 2);
    if (ret == ESP_OK) {
...
esp_err_t tca9535_write_register_pair(tca9535_handle_t handle, tca9535_reg_t reg, const tca9535_register_t *data)
{
    ...
    esp_err_t ret = i2c_dev_write_reg(&dev->i2c_dev, reg, write_data, 2);
    if (ret != ESP_OK) {
