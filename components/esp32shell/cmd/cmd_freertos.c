...
void task_queue(uint32_t channel_id, const char *params) {
    ...
    if (sscanf(params, "%31s %31s", cmd, arg) < 1) {
        shell_snprintf(response, sizeof(response), "用法: queue <create|send|receive>\r\n");
    ...
    if (strcmp(cmd, "create") == 0) {
        ...
        if (demo_queue != NULL) {
            shell_snprintf(response, sizeof(response), "队列创建成功，大小: %d\r\n", size);
        } else {
            shell_snprintf(response, sizeof(response), "队列创建失败\r\n");
        }
    } else if (strcmp(cmd, "send") == 0) {
        if (demo_queue == NULL) {
            shell_snprintf(response, sizeof(response), "错误: 队列未创建\r\n");
        } else {
            int value = strlen(arg) > 0 ? atoi(arg) : 42; // 默认值42
            if (xQueueSend(demo_queue, &value, pdMS_TO_TICKS(1000)) == pdTRUE) {
                shell_snprintf(response, sizeof(response), "成功发送数据: %d\r\n", value);
            } else {
                shell_snprintf(response, sizeof(response), "发送数据失败\r\n");
            }
        }
        ...
        if (demo_queue == NULL) {
            ...
            if (xQueueReceive(demo_queue, &value, pdMS_TO_TICKS(1000)) == pdTRUE) {
                shell_snprintf(response, sizeof(response), "接收到数据: %d\r\n", value);
            } else {
                shell_snprintf(response, sizeof(response), "接收数据失败\r\n");
            }
        }
    } else {
...
void task_sem(uint32_t channel_id, const char *params) {
    ...
    if (sscanf(params, "%31s", cmd) < 1) {
        shell_snprintf(response, sizeof(response), "用法: sem <create|take|give>\r\n");
        cmd_output(channel_id, (uint8_t *)response, strlen(response));
        return;
    }
    ...
    if (strcmp(cmd, "create") == 0) {
        ...
        if (demo_sem != NULL) {
            shell_snprintf(response, sizeof(response), "信号量创建成功\r\n");
        } else {
            shell_snprintf(response, sizeof(response), "信号量创建失败\r\n");
        }
    } else if (strcmp(cmd, "take") == 0) {
        if (demo_sem == NULL) {
            shell_snprintf(response, sizeof(response), "错误: 信号量未创建\r\n");
        } else {
            if (xSemaphoreTake(demo_sem, pdMS_TO_TICKS(1000)) == pdTRUE) {
                shell_snprintf(response, sizeof(response), "成功获取信号量\r\n");
            } else {
                shell_snprintf(response, sizeof(response), "获取信号量失败\r\n");
            }
        }
        ...
        if (demo_sem == NULL) {
            ...
            if (xSemaphoreGive(demo_sem) == pdTRUE) {
                shell_snprintf(response, sizeof(response), "成功释放信号量\r\n");
            } else {
                shell_snprintf(response, sizeof(response), "释放信号量失败\r\n");
            }
        }
    } else {
        shell_snprintf(response, sizeof(response), "未知命令: %s\r\n", cmd);
...
void task_timer(uint32_t channel_id, const char *params) {
    ...
    if (sscanf(params, "%31s %31s", cmd, arg) < 1) {
        shell_snprintf(response, sizeof(response), "用法: timer <create|start|stop>\r\n");
        cmd_output(channel_id, (uint8_t *)response, strlen(response));
        return;
    }
    ...
    if (strcmp(cmd, "create") == 0) {
        ...
        if (demo_timer != NULL) {
            shell_snprintf(response, sizeof(response), "定时器创建成功，周期: %d ms\r\n", period);
        } else {
            shell_snprintf(response, sizeof(response), "定时器创建失败\r\n");
        }
    } else if (strcmp(cmd, "start") == 0) {
        if (demo_timer == NULL) {
            shell_snprintf(response, sizeof(response), "错误: 定时器未创建\r\n");
        } else {
            if (xTimerStart(demo_timer, pdMS_TO_TICKS(1000)) == pdTRUE) {
                shell_snprintf(response, sizeof(response), "定时器启动成功\r\n");
            } else {
                shell_snprintf(response, sizeof(response), "定时器启动失败\r\n");
            }
        }
        ...
        if (demo_timer == NULL) {
            ...
            if (xTimerStop(demo_timer, pdMS_TO_TICKS(1000)) == pdTRUE) {
                shell_snprintf(response, sizeof(response), "定时器停止成功\r\n");
            } else {
                shell_snprintf(response, sizeof(response), "定时器停止失败\r\n");
            }
        }
    } else {
        shell_snprintf(response, sizeof(response), "未知命令: %s\r\n", cmd);
